...
export const Header = () => {
  ...
  return <header className="sticky top-0 z-40 border-b border-border/20 bg-background/60 backdrop-blur-xl">
        ...
        <div className="flex items-center justify-between">
          ...
          x: 0
            ...
            <Link to="/" className="flex items-center gap-2.5">
              <motion.div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-primary via-primary to-info flex items-center justify-center shadow-lg" whileHover={{
              scale: 1.05,
              rotate: 3
            }} whileTap={{
              scale: 0.95
                ...
                <motion.div className="absolute -top-0.5 -right-0.5" animate={{
                scale: [1, 1.2, 1],
                rotate: [0, 10, -10, 0]
              }} transition={{
                duration: 3,
              ...
              <div className="hidden sm:block">
                <h1 className="text-base font-display font-bold gradient-text leading-tight">BetterMe</h1>
          ...
          <nav className="hidden md:flex items-center gap-0.5 bg-secondary/30 rounded-xl p-1">
            ...
            return <Link key={item.href} to={item.href}>
                ...
                scale: 0.98
                    ...
                    <Button variant={isActive ? "default" : "ghost"} size="sm" className={`gap-1.5 h-8 px-3 text-xs font-medium transition-all ${isActive ? 'bg-primary text-primary-foreground shadow-md' : 'hover:bg-secondary/80'}`}>
          ...
          x: 0
            ...
            <motion.div className="hidden sm:flex items-center gap-3 px-3 py-1.5 rounded-xl glass border border-primary/10" whileHover={{
            scale: 1.01
          }}>
              <span className="text-xs text-muted-foreground">{formattedDate}</span>
              <div className="w-px h-4 bg-border/50" />
              <motion.span className="text-sm font-display font-bold text-foreground tabular-nums" key={timeString} initial={{
              opacity: 0.5,
              y: -2
            }} animate={{
              opacity: 1,
